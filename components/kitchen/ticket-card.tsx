...
export function TicketCard({ order, onStatusChange, onOrderComplete }: TicketCardProps) {
  ...
  const [itemStatuses, setItemStatuses] = useState<Record<string, ItemStatus>>(() =>
    Object.fromEntries(order.items.map(item => [item.id, "pending" as ItemStatus]))
  );
  ...
  const hasPendingItems = order.items.some(item => itemStatuses[item.id] === "pending");
  const hasPreparingItems = order.items.some(item => itemStatuses[item.id] === "preparing");
  ...
  const handleItemClick = (item: OrderItem) => {
    const current = itemStatuses[item.id] ?? "pending";
    let next: ItemStatus = "preparing";
    if (current === "preparing") next = "ready";
    if (current === "ready") next = "served";
    if (current !== "served") {
      setItemStatuses(prev => ({ ...prev, [item.id]: next }));
      onStatusChange(order.id, item.id, next);
    }
  };
            ...
            return (
              <motion.div
                ...
                className={`
                  ...
                  ${status === 'ready' ? 'bg-success/10 border-success/30 opacity-70' : ''}
                  ${status === 'preparing' ? 'bg-info/10 border-info/30 shadow-sm' : ''}
                  ${status === 'pending' ? 'bg-background border-border hover:border-primary/30' : ''}
                `}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-semibold text-lg ${status === 'ready' ? 'line-through text-muted-foreground' : ''}`}>
                        {item.itemName}
                      </span>
                    </div>
                    ...
                    {item.modifiers && item.modifiers.length > 0 && (
                      <ul className="mt-1 ml-6 text-sm text-muted-foreground list-disc">
                        {item.modifiers.map((mod: { name: string }, i: number) => (
                  ...
                  <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-background border shadow-sm">
                    ...
                    {status === 'preparing' && <ChefHat className="w-4 h-4 text-info animate-pulse" />}
                    {status === 'ready' && <Check className="w-4 h-4 text-success" />}
                  </div>
                </div>
              </motion.div>
            );
